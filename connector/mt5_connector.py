...
def err(message,status=400): return jsonify({"ok":False,"error":message,"mt5_error":mt5.last_error()}),status
def authorized(): return request.headers.get('X-Signal-Pro-Pair')==PAIR_TOKEN
def guard(): return None if authorized() else (jsonify({"ok":False,"error":"Connector não pareado"}),401)
def account_payload():
 a=mt5.account_info()
...
@app.get('/api/pair')
def pair(): return jsonify({"ok":True,"pair_token":PAIR_TOKEN,"message":"Use este código somente no Signal Pro desta máquina."}) if request.remote_addr in ('127.0.0.1','::1') else err('Pareamento disponível apenas localmente',403)
@app.get('/api/health')
def health():
 g=guard()
 if g:return g
 a=account_payload();return jsonify({"ok":True,"mt5_connected":a is not None,"account":a})
@app.get('/api/broker-hint')
def broker_hint():
 g=guard()
 if g:return g
 q=(request.args.get('q') or '').strip().lower();a=account_payload();h=[]
 if a:
  text=' '.join([str(a.get('company') or ''),str(a.get('server') or '')])
  if not q or q in text.lower():h.append({"broker":a.get('company') or 'Corretora MT5',"server":a.get('server'),"source":"terminal"})
 return jsonify({"ok":True,"hints":h,"note":"O servidor exato é definido pelo terminal/corretora; o Connector não inventa servidores."})
...
def symbols():
 ...
 data=[{"name":s.name,"description":s.description,"path":s.path,"visible":bool(s.visible),"digits":s.digits,"point":s.point,"volume_min":s.volume_min,"volume_max":s.volume_max,"volume_step":s.volume_step,"trade_contract_size":s.trade_contract_size,"currency_base":s.currency_base,"currency_profit":s.currency_profit,"currency_margin":s.currency_margin} for s in xs]
...
if __name__=='__main__':
 ...
 print('Pareamento local:',PAIR_TOKEN)
 print('Abra o Signal Pro e informe este código. A senha MT5 não é gravada pelo Connector.\n')
